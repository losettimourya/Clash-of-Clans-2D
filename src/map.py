...
def printMap(V):
    ...
    for arch in stealtharchers:
        ...
        map_matrix[a+1][b] = stealtharchColor(arch) + Fore.BLACK + 'LT'
        map_matrix[a][b+1] = stealtharchColor(arch) + Fore.BLACK + 'EA'
        map_matrix[a+1][b+1] = stealtharchColor(arch) + Fore.BLACK + 'H '
...
def showKingHealth(health, max_health):
    health_bar = []
    health_scaled = int((health*10)/max_health)
    for i in range(10):
        if(i<health_scaled):
