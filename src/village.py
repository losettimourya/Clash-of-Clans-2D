...
class Village:
    ...
    def generate_map(self):
        ...
        for wall in self.walls_top:                                     # wall is 1x1
            wall_obj = bd.Wall(wall, self, level= 2*self.level - 1)
        ...
        for cannon in self.cannons:                                     # cannon is 2x2
            cannon_obj = bd.Cannon(cannon, self, level=2*self.level - 1)
        ...
        for wizard_tower in self.wizard_towers:                          # wizard tower is 1X1
            wizard_tower_obj = bd.WizardTower(wizard_tower, self, level=2*self.level - 1)
    ...
    def remove_wall(self, wall_obj, King):
        ...
        if wall_obj.level >= 3:
            for troop in troops:
                if( abs(troop.position[0] - wall_obj.position[0]) <= wall_obj.explosion_radius and abs(troop.position[1] - wall_obj.position[1]) <= wall_obj.explosion_radius):
                    troop.health -= wall_obj.explosion_damage
                    if troop.health <= 0:
                        troop.health = 0
                        troop.kill()
            #King = king.getHero(pt.hero)
            #print(King.position)
            if( abs(King.position[0] - wall_obj.position[0]) <= wall_obj.explosion_radius  and abs(King.position[1] - wall_obj.position[1]) <= wall_obj.explosion_radius):
                    King.wall_damage(wall_obj.explosion_damage)
